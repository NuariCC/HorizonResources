...
function AngryAssign_ToggleIcons()
	if AngryAssign.iconpicker:IsShown() then 
		AngryAssign.iconpicker:Hide() 
	else
		AngryAssign.iconpicker:Show() 
	end
end

function AngryAssign_ToggleColors()
	if AngryAssign.colorpicker:IsShown() then 
		AngryAssign.colorpicker:Hide() 
	else
		AngryAssign.colorpicker:Show() 
	end
end
...
function AngryAssign:CreateWindow()
	...
	window.frame:SetMinResize(900, 400)
	...
	local button_icons = AceGUI:Create("Button")
	button_icons:SetText("Icons")
	button_icons:SetWidth(80)
	button_icons:SetHeight(22)
	button_icons:ClearAllPoints()
	button_icons:SetPoint("BOTTOMLEFT", button_output.frame, "BOTTOMRIGHT", 6, 0)
	button_icons:SetCallback("OnClick", AngryAssign_ToggleIcons)
	tree:AddChild(button_icons)
	window.button_icons = button_icons

	local button_colors = AceGUI:Create("Button")
	button_colors:SetText("Colors")
	button_colors:SetWidth(80)
	button_colors:SetHeight(22)
	button_colors:ClearAllPoints()
	button_colors:SetPoint("BOTTOMLEFT", button_icons.frame, "BOTTOMRIGHT", 6, 0)
	button_colors:SetCallback("OnClick", AngryAssign_ToggleColors)
	tree:AddChild(button_colors)
...
local function AngryAssign_IconPicker_Clicked(widget, event)
	...
	if position > 0 then
		...
		AngryAssign.window.text:SetText( strsub(text, 1, position)..icon..strsub(text, position+1, string.len(text) ))
		AngryAssign.window.text.editBox:SetCursorPosition( position + string.len(icon), string.len(text) )
	else
		AngryAssign.window.text:SetText( icon..AngryAssign.window.text:GetText() )
		AngryAssign.window.text.editBox:SetCursorPosition( string.len(icon) )
	end

	AngryAssign.window.text.button:Enable()
	AngryAssign_TextChanged()
	AngryAssign.window.text.editBox:SetFocus()
end

local iconCache = nil
local function AngryAssign_IconPicker_TextChanged(widget, event, value)
	AngryAssign.iconpicker_group:ReleaseChildren()
	...
	local names = {}
	local variant = AngryAssign.src_dropdown_choice

	if variant == 1 then
		local path = select(3, GetSpellInfo(tonumber(value)))
		tinsert(names, path)
	elseif variant == 2 then
		local path = select(10, GetItemInfo(tonumber(value)))
		tinsert(names, path)
	end
	...
	for _, path in ipairs(names) do
		if path then
			...
			AngryAssign.iconpicker_group:AddChild(icon)
		end
	end
end

local function AngryAssign_IconPicker_OnValueChanged(widget, event, value)
	AngryAssign.src_dropdown_choice = value
	AngryAssign_IconPicker_TextChanged(AngryAssign.iconpicker.source_id_editbox, "OnTextChanged", AngryAssign.iconpicker.source_id_editbox:GetText())
end
...
function AngryAssign:CreateIconRegionButton(name, texture, left, right, top, bottom)
	local icon = AceGUI:Create("Icon")
	icon:SetImage(texture, left / 64, right / 64, top / 64, bottom / 64)
	icon:SetImageSize(20, 20)
	icon:SetWidth(21)
	icon:SetHeight(24)
	icon:SetUserData('name', name)
...
function AngryAssign:CreateIconPicker()
	...
	window.frame:SetPoint("TOPLEFT", self.window.frame, "TOPRIGHT", 4, 0)
	...
	local group_general = AceGUI:Create("SimpleGroup")
	group_general:SetLayout("Flow")
	group_general:SetFullWidth(true)
	for i = 8, 1, -1 do 
		group_general:AddChild( self:CreateIconButton("{rt"..i.."}", "Interface\\TargetingFrame\\UI-RaidTargetingIcon_"..i) )
	end
	group_general:AddChild( self:CreateIconRegionButton("{dps}", "Interface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES", 20, 39, 22, 41) )
	group_general:AddChild( self:CreateIconRegionButton("{healer}", "Interface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES", 20, 39, 1, 20) )
	group_general:AddChild( self:CreateIconRegionButton("{tank}", "Interface\\LFGFrame\\UI-LFG-ICON-PORTRAITROLES", 0, 19, 22, 41) )
	group_general:AddChild( self:CreateIconButton("{icon ability_creature_cursed_02}", "Interface\\Icons\\ability_creature_cursed_02") ) -- Danger
	

	window:AddChild(group_general)

	-- Hunter stuff
	local group_hunter = AceGUI:Create("SimpleGroup")
	group_hunter:SetLayout("Flow")
	group_hunter:SetFullWidth(true)

	group_hunter:AddChild( self:CreateIconRegionButton("{hunter}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 0, 16, 16, 32) )
	group_hunter:AddChild( self:CreateIconButton("{icon ability_hunter_misdirection}", "Interface\\Icons\\ability_hunter_misdirection") ) -- Misdirection
	group_hunter:AddChild( self:CreateIconButton("{icon ability_rogue_feigndeath}", "Interface\\Icons\\ability_rogue_feigndeath") ) -- Feign Death
	group_hunter:AddChild( self:CreateIconButton("{icon ability_whirlwind}", "Interface\\Icons\\ability_whirlwind") ) -- Deterrence
	group_hunter:AddChild( self:CreateIconButton("{icon ability_rogue_feint}", "Interface\\Icons\\ability_rogue_feint") ) -- Disengage
	group_hunter:AddChild( self:CreateIconButton("{icon spell_frost_chainsofice}", "Interface\\Icons\\spell_frost_chainsofice") ) -- Freezing Trap
	group_hunter:AddChild( self:CreateIconButton("{icon ability_mount_whitetiger}", "Interface\\Icons\\ability_mount_whitetiger") ) -- Aspect of the Pack
	window:AddChild(group_hunter)

	-- Warr stuff
	local group_warrior = AceGUI:Create("SimpleGroup")
	group_warrior:SetLayout("Flow")
	group_warrior:SetFullWidth(true)

	group_warrior:AddChild( self:CreateIconRegionButton("{warrior}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 0, 16, 0, 16) )
	group_warrior:AddChild( self:CreateIconButton("{icon spell_nature_reincarnation}", "Interface\\Icons\\spell_nature_reincarnation") ) -- Taunt
	group_warrior:AddChild( self:CreateIconButton("{icon ability_warrior_shieldwall}", "Interface\\Icons\\ability_warrior_shieldwall") ) -- Shield Wall
	group_warrior:AddChild( self:CreateIconButton("{icon spell_holy_ashestoashes}", "Interface\\Icons\\spell_holy_ashestoashes") ) -- Last Stand
	group_warrior:AddChild( self:CreateIconButton("{icon ability_golemthunderclap}", "Interface\\Icons\\ability_golemthunderclap") ) -- Intimidating Shout
	group_warrior:AddChild( self:CreateIconButton("{icon ability_bullrush}", "Interface\\Icons\\ability_bullrush") ) -- Challenging Shout
	group_warrior:AddChild( self:CreateIconButton("{icon spell_nature_ancestralguardian}", "Interface\\Icons\\spell_nature_ancestralguardian") ) -- Berserker Rage
	group_warrior:AddChild( self:CreateIconButton("{icon ability_warrior_victoryrush}", "Interface\\Icons\\ability_warrior_victoryrush") ) -- Intervene
	group_warrior:AddChild( self:CreateIconButton("{icon ability_warrior_shieldreflection}", "Interface\\Icons\\ability_warrior_shieldreflection") ) -- Spell Reflection
	group_warrior:AddChild( self:CreateIconButton("{icon ability_warrior_shieldbash}", "Interface\\Icons\\ability_warrior_shieldbash") ) -- Shield Bash
	group_warrior:AddChild( self:CreateIconButton("{icon ability_warrior_disarm}", "Interface\\Icons\\ability_warrior_disarm") )  -- Disarm
	window:AddChild(group_warrior)

	-- Rogue stuff
	local group_rogue = AceGUI:Create("SimpleGroup")
	group_rogue:SetLayout("Flow")
	group_rogue:SetFullWidth(true)

	group_rogue:AddChild( self:CreateIconRegionButton("{rogue}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 32, 48, 0, 16) )
	group_rogue:AddChild( self:CreateIconButton("{icon ability_rogue_tricksofthetrade}", "Interface\\Icons\\ability_rogue_tricksofthetrade") ) -- Tricks of the Trade
	group_rogue:AddChild( self:CreateIconButton("{icon ability_kick}", "Interface\\Icons\\ability_kick") ) -- Kick
	group_rogue:AddChild( self:CreateIconButton("{icon ability_rogue_dismantle}", "Interface\\Icons\\ability_rogue_dismantle") ) -- Dismantle
	group_rogue:AddChild( self:CreateIconButton("{icon ability_rogue_kidneyshot}", "Interface\\Icons\\ability_rogue_kidneyshot") ) -- Kidney Shot
	group_rogue:AddChild( self:CreateIconButton("{icon spell_shadow_mindsteal}", "Interface\\Icons\\spell_shadow_mindsteal") ) -- Blind
	group_rogue:AddChild( self:CreateIconButton("{icon ability_rogue_distract}", "Interface\\Icons\\ability_rogue_distract") ) -- Distract
	group_rogue:AddChild( self:CreateIconButton("{icon spell_shadow_shadowward}", "Interface\\Icons\\spell_shadow_shadowward") ) -- Evasion
	group_rogue:AddChild( self:CreateIconButton("{icon spell_shadow_nethercloak}", "Interface\\Icons\\spell_shadow_nethercloak") ) -- Cloak of Shadows
	group_rogue:AddChild( self:CreateIconButton("{icon ability_vanish}", "Interface\\Icons\\ability_vanish") ) -- Vanish
	group_rogue:AddChild( self:CreateIconButton("{icon spell_shadow_grimward}", "Interface\\Icons\\spell_shadow_grimward") ) -- Disarm Trap
	
	window:AddChild(group_rogue)

	-- Mage stuff
	local group_mage = AceGUI:Create("SimpleGroup")
	group_mage:SetLayout("Flow")
	group_mage:SetFullWidth(true)

	group_mage:AddChild( self:CreateIconRegionButton("{mage}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 16, 32, 0, 16) )
	group_mage:AddChild( self:CreateIconButton("{icon spell_frost_frost}", "Interface\\Icons\\spell_frost_frost") ) -- Ice Block
	group_mage:AddChild( self:CreateIconButton("{icon spell_arcane_arcane02}", "Interface\\Icons\\spell_arcane_arcane02") ) -- Spellsteal
	group_mage:AddChild( self:CreateIconButton("{icon spell_nature_polymorph}", "Interface\\Icons\\spell_nature_polymorph") ) -- Polymorph
	group_mage:AddChild( self:CreateIconButton("{icon spell_frost_iceshock}", "Interface\\Icons\\spell_frost_iceshock") ) -- Counterspell
	group_mage:AddChild( self:CreateIconButton("{icon spell_nature_removecurse}", "Interface\\Icons\\spell_nature_removecurse") ) -- Remove Curse
	group_mage:AddChild( self:CreateIconButton("{icon ability_mage_invisibility}", "Interface\\Icons\\ability_mage_invisibility") ) -- Invisibility
	group_mage:AddChild( self:CreateIconButton("{icon spell_frost_frostnova}", "Interface\\Icons\\spell_frost_frostnova") ) -- Frost Nova
	group_mage:AddChild( self:CreateIconButton("{icon spell_frost_icestorm}", "Interface\\Icons\\spell_frost_icestorm") ) -- Blizzard
	group_mage:AddChild( self:CreateIconButton("{icon spell_nature_abolishmagic}", "Interface\\Icons\\spell_nature_abolishmagic") ) -- Dampen Magic
	group_mage:AddChild( self:CreateIconButton("{icon spell_holy_flashheal}", "Interface\\Icons\\spell_holy_flashheal") ) -- Amplify Magic
	window:AddChild(group_mage)

	-- Priest stuff
	local group_priest = AceGUI:Create("SimpleGroup")
	group_priest:SetLayout("Flow")
	group_priest:SetFullWidth(true)

	group_priest:AddChild( self:CreateIconRegionButton("{priest}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 32, 48, 16, 32) )
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_powerinfusion}", "Interface\\Icons\\spell_holy_powerinfusion") ) -- Power Infusion
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_painsupression}", "Interface\\Icons\\spell_holy_painsupression") ) -- Pain Suppression
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_divinehymn}", "Interface\\Icons\\spell_holy_divinehymn") ) -- Divine Hymn
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_symbolofhope}", "Interface\\Icons\\spell_holy_symbolofhope") ) -- Hymn of Hope
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_guardianspirit}", "Interface\\Icons\\spell_holy_guardianspirit") ) -- Guardian Spirit
	group_priest:AddChild( self:CreateIconButton("{icon spell_shadow_dispersion}", "Interface\\Icons\\spell_shadow_dispersion") ) -- Dispersion
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_excorcism}", "Interface\\Icons\\spell_holy_excorcism") ) -- Fear Ward
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_powerwordshield}", "Interface\\Icons\\spell_holy_powerwordshield") ) -- PW:S
	group_priest:AddChild( self:CreateIconButton("{icon spell_shadow_psychicscream}", "Interface\\Icons\\spell_shadow_psychicscream") ) -- Psychic Scream
	group_priest:AddChild( self:CreateIconButton("{icon spell_nature_nullifydisease}", "Interface\\Icons\\spell_nature_nullifydisease") ) -- Abolish Disease
	group_priest:AddChild( self:CreateIconButton("{icon spell_holy_dispelmagic}", "Interface\\Icons\\spell_holy_dispelmagic") ) -- Dispel Magic
	group_priest:AddChild( self:CreateIconButton("{icon spell_shadow_manaburn}", "Interface\\Icons\\spell_shadow_manaburn") ) -- Mana Burn
	group_priest:AddChild( self:CreateIconButton("{icon spell_arcane_massdispel}", "Interface\\Icons\\spell_arcane_massdispel") ) -- Mass Dispell
	group_priest:AddChild( self:CreateIconButton("{icon spell_nature_slow}", "Interface\\Icons\\spell_nature_slow") ) -- Shackle Undead
	window:AddChild(group_priest)

	-- Warlock stuff
	local group_warlock = AceGUI:Create("SimpleGroup")
	group_warlock:SetLayout("Flow")
	group_warlock:SetFullWidth(true)

	group_warlock:AddChild( self:CreateIconRegionButton("{warlock}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 48, 64, 16, 32) )
	group_warlock:AddChild( self:CreateIconButton("{icon spell_shadow_soulgem}", "Interface\\Icons\\spell_shadow_soulgem") ) -- Soulstone
	group_warlock:AddChild( self:CreateIconButton("{hs}", "Interface\\Icons\\INV_Stone_04") ) -- Healthstone
	group_warlock:AddChild( self:CreateIconButton("{icon spell_shadow_possession}", "Interface\\Icons\\spell_shadow_possession") ) -- Fear
	group_warlock:AddChild( self:CreateIconButton("{icon spell_fire_soulburn}", "Interface\\Icons\\spell_fire_soulburn") ) -- Searing Pain
	group_warlock:AddChild( self:CreateIconButton("{icon spell_shadow_cripple}", "Interface\\Icons\\spell_shadow_cripple") ) -- Banish
	group_warlock:AddChild( self:CreateIconButton("{icon spell_shadow_curseoftounges}", "Interface\\Icons\\spell_shadow_curseoftounges") ) -- Curse of Tongues
	group_warlock:AddChild( self:CreateIconButton("{icon spell_shadow_siphonmana}", "Interface\\Icons\\spell_shadow_siphonmana") ) -- Drain Mana
	group_warlock:AddChild( self:CreateIconButton("{icon spell_shadow_deathscream}", "Interface\\Icons\\spell_shadow_deathscream") ) -- Howl of Terror
	group_warlock:AddChild( self:CreateIconButton("{icon spell_shadow_shadowfury}", "Interface\\Icons\\spell_shadow_shadowfury") ) -- Shadowfury
	window:AddChild(group_warlock)

	-- Paladin stuff
	local group_paladin = AceGUI:Create("SimpleGroup")
	group_paladin:SetLayout("Flow")
	group_paladin:SetFullWidth(true)

	group_paladin:AddChild( self:CreateIconRegionButton("{paladin}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 0, 16, 32, 48) )
	group_paladin:AddChild( self:CreateIconButton("{icon inv_shoulder_37}", "Interface\\Icons\\inv_shoulder_37") ) -- Righteous Defense
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_divineintervention}", "Interface\\Icons\\spell_holy_divineintervention") ) -- Divine Shield
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_sealofprotection}", "Interface\\Icons\\spell_holy_sealofprotection") ) -- Hand of Protection
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_restoration}", "Interface\\Icons\\spell_holy_restoration") ) -- Divine Protection
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_layonhands}", "Interface\\Icons\\spell_holy_layonhands") ) -- Lay on Hands
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_auramastery}", "Interface\\Icons\\spell_holy_auramastery") ) -- Aura Mastery
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_sealofsalvation}", "Interface\\Icons\\spell_holy_sealofsalvation") ) -- Hand of Salvation
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_sealofsacrifice}", "Interface\\Icons\\spell_holy_sealofsacrifice") ) -- Hand of Sacrifice
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_sealofvalor}", "Interface\\Icons\\spell_holy_sealofvalor") ) -- Hand of Freedom
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_powerwordbarrier}", "Interface\\Icons\\spell_holy_powerwordbarrier") ) -- Divine Sacrifice
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_sealofmight}", "Interface\\Icons\\spell_holy_sealofmight") ) -- Hammer of Justice
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_prayerofhealing}", "Interface\\Icons\\spell_holy_prayerofhealing") ) -- Repentance
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_turnundead}", "Interface\\Icons\\spell_holy_turnundead") ) -- Turn Evil
	group_paladin:AddChild( self:CreateIconButton("{icon spell_holy_excorcism}", "Interface\\Icons\\spell_holy_excorcism") ) -- Holy Wrath
	window:AddChild(group_paladin)

	-- Druid stuff
	local group_druid = AceGUI:Create("SimpleGroup")
	group_druid:SetLayout("Flow")
	group_druid:SetFullWidth(true)

	group_druid:AddChild( self:CreateIconRegionButton("{druid}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 48, 64, 0, 16) )
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_tranquility}", "Interface\\Icons\\spell_nature_tranquility") ) -- Tranquility
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_lightning}", "Interface\\Icons\\spell_nature_lightning") ) -- Innervate
	group_druid:AddChild( self:CreateIconButton("{icon ability_druid_tigersroar}", "Interface\\Icons\\ability_druid_tigersroar") ) -- Survival Instincts
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_stoneclawtotem}", "Interface\\Icons\\spell_nature_stoneclawtotem") ) -- Barkskin
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_earthbind}", "Interface\\Icons\\spell_nature_earthbind") ) -- Cyclone
	group_druid:AddChild( self:CreateIconButton("{icon ability_druid_bash}", "Interface\\Icons\\ability_druid_bash") ) -- Bash
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_stranglevines}", "Interface\\Icons\\spell_nature_stranglevines") ) -- Entangling Roots
	group_druid:AddChild( self:CreateIconButton("{icon ability_physical_taunt}", "Interface\\Icons\\ability_physical_taunt") ) -- Growl
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_sleep}", "Interface\\Icons\\spell_nature_sleep") ) -- Hibernate
	group_druid:AddChild( self:CreateIconButton("{icon spell_holy_removecurse}", "Interface\\Icons\\spell_holy_removecurse") ) -- Remove Curse
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_nullifypoison_02}", "Interface\\Icons\\spell_nature_nullifypoison_02") ) -- Abolish Poison
	group_druid:AddChild( self:CreateIconButton("{icon spell_nature_reincarnation}", "Interface\\Icons\\spell_nature_reincarnation") ) -- Rebirth
	group_druid:AddChild( self:CreateIconButton("{icon ability_druid_challangingroar}", "Interface\\Icons\\ability_druid_challangingroar") ) -- Challenging Roar
	group_druid:AddChild( self:CreateIconButton("{icon ability_bullrush}", "Interface\\Icons\\ability_bullrush") ) -- Frenzied Regeneration
	window:AddChild(group_druid)

	-- Shaman stuff
	local group_shaman = AceGUI:Create("SimpleGroup")
	group_shaman:SetLayout("Flow")
	group_shaman:SetFullWidth(true)

	group_shaman:AddChild( self:CreateIconRegionButton("{shaman}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 16, 32, 16, 32) )
	group_shaman:AddChild( self:CreateIconButton("{bl}", "Interface\\Icons\\SPELL_Nature_Bloodlust") ) -- Bloodlust
	group_shaman:AddChild( self:CreateIconButton("{heroism}", "Interface\\Icons\\ABILITY_Shaman_Heroism") ) -- Heroism
	group_shaman:AddChild( self:CreateIconButton("{icon spell_frost_summonwaterelemental}", "Interface\\Icons\\spell_frost_summonwaterelemental") ) -- Mana Tide Totem
	group_shaman:AddChild( self:CreateIconButton("{icon spell_nature_tremortotem}", "Interface\\Icons\\spell_nature_tremortotem") ) -- Tremor Totem
	group_shaman:AddChild( self:CreateIconButton("{icon spell_nature_groundingtotem}", "Interface\\Icons\\spell_nature_groundingtotem") ) -- Grounding Totem
	group_shaman:AddChild( self:CreateIconButton("{icon spell_nature_cyclone}", "Interface\\Icons\\spell_nature_cyclone") ) -- Wind Shear
	group_shaman:AddChild( self:CreateIconButton("{icon ability_druid_bash}", "Interface\\Icons\\ability_druid_bash") ) -- Bash
	group_shaman:AddChild( self:CreateIconButton("{icon spell_shaman_hex}", "Interface\\Icons\\spell_shaman_hex") ) -- Hex
	group_shaman:AddChild( self:CreateIconButton("{icon spell_nature_diseasecleansingtotem}", "Interface\\Icons\\spell_nature_diseasecleansingtotem") ) -- Cleansing Totem
	group_shaman:AddChild( self:CreateIconButton("{icon spell_nature_purge}", "Interface\\Icons\\spell_nature_purge") ) -- Purge
	group_shaman:AddChild( self:CreateIconButton("{icon spell_nature_nullifypoison}", "Interface\\Icons\\spell_nature_nullifypoison") ) -- Cure Toxins
	group_shaman:AddChild( self:CreateIconButton("{icon spell_nature_reincarnation}", "Interface\\Icons\\spell_nature_reincarnation") ) -- Reincarnation
	group_shaman:AddChild( self:CreateIconButton("{icon spell_shaman_thunderstorm}", "Interface\\Icons\\spell_shaman_thunderstorm") ) -- Thunderstorm
	window:AddChild(group_shaman)

	-- DK stuff
	local group_dk = AceGUI:Create("SimpleGroup")
	group_dk:SetLayout("Flow")
	group_dk:SetFullWidth(true)

	group_dk:AddChild( self:CreateIconRegionButton("{deathknight}", "Interface\\GLUES\\CHARACTERCREATE\\UI-CHARACTERCREATE-CLASSES", 16, 32, 32, 48) )
	group_dk:AddChild( self:CreateIconButton("{icon spell_deathknight_antimagiczone}", "Interface\\Icons\\spell_deathknight_antimagiczone") ) -- AMZ
	group_dk:AddChild( self:CreateIconButton("{icon spell_shadow_antimagicshell}", "Interface\\Icons\\spell_shadow_antimagicshell") ) -- AMS
	group_dk:AddChild( self:CreateIconButton("{icon spell_deathknight_iceboundfortitude}", "Interface\\Icons\\spell_deathknight_iceboundfortitude") ) -- Icebound Fortitude
	group_dk:AddChild( self:CreateIconButton("{icon spell_shadow_lifedrain}", "Interface\\Icons\\spell_shadow_lifedrain") ) -- Vampiric Blood
	group_dk:AddChild( self:CreateIconButton("{icon spell_frost_chainsofice}", "Interface\\Icons\\spell_frost_chainsofice") ) -- Chains of Ice
	group_dk:AddChild( self:CreateIconButton("{icon spell_deathknight_mindfreeze}", "Interface\\Icons\\spell_deathknight_mindfreeze") ) -- Mind Freeze
	group_dk:AddChild( self:CreateIconButton("{icon spell_shadow_soulleech_3}", "Interface\\Icons\\spell_shadow_soulleech_3") ) -- Strangulate
	group_dk:AddChild( self:CreateIconButton("{icon spell_deathknight_strangulate}", "Interface\\Icons\\spell_deathknight_strangulate") ) -- Death Grip
	window:AddChild(group_dk)

	-- Resistance stuff
	local group_resistance = AceGUI:Create("SimpleGroup")
	group_resistance:SetLayout("Flow")
	group_resistance:SetFullWidth(true)

	group_resistance:AddChild( self:CreateIconButton("{icon spell_nature_wispsplode}", "Interface\\Icons\\spell_nature_wispsplode") ) -- Arcane Resistance
	group_resistance:AddChild( self:CreateIconButton("{icon spell_fire_sealoffire}", "Interface\\Icons\\spell_fire_sealoffire") ) -- Fire Resistance
	group_resistance:AddChild( self:CreateIconButton("{icon spell_nature_natureresistancetotem}", "Interface\\Icons\\spell_nature_natureresistancetotem") ) -- Nature Resistance
	group_resistance:AddChild( self:CreateIconButton("{icon spell_frost_wizardmark}", "Interface\\Icons\\spell_frost_wizardmark") ) -- Frost Resistance
	group_resistance:AddChild( self:CreateIconButton("{icon spell_shadow_sealofkings}", "Interface\\Icons\\spell_shadow_sealofkings") ) -- Shadow Resistance
	
	window:AddChild(group_resistance)

	local heading = AceGUI:Create("Heading")
	heading:SetFullWidth(true)
	window:AddChild(heading)

	local src_dropdown = AceGUI:Create("Dropdown")
	src_dropdown:SetFullWidth(true)
	src_dropdown:SetLabel("Source:")
	src_dropdown:SetList({
		[1] = "Spell ID",
		[2] = "Item ID",
	})
	src_dropdown:SetValue(1)
	self.src_dropdown_choice = 1
	src_dropdown:SetCallback("OnValueChanged", AngryAssign_IconPicker_OnValueChanged)
	window:AddChild(src_dropdown)

	local text = AceGUI:Create("EditBox")
	text:SetFullWidth(true)
	text:DisableButton(true)
	text:SetCallback("OnTextChanged", AngryAssign_IconPicker_TextChanged)
	self.iconpicker.source_id_editbox = text
	window:AddChild(text)

	local icon_group = AceGUI:Create("SimpleGroup")
	icon_group:SetLayout("Flow")
	icon_group:SetFullWidth(true)
	icon_group:SetFullHeight(true)
	window:AddChild(icon_group)
	self.iconpicker_group = icon_group
end

local function AngryAssign_ColorPicker_Clicked(widget, event)
	local page = AngryAssign_Pages[ AngryAssign:SelectedId() ]
	if not page then
		return
	end
	...
	local color
	if widget:GetUserData('color') then
		color = widget:GetUserData('color')
	else
		return
	end
	...
	if position > 0 then
		local text = AngryAssign.window.text:GetText()
		AngryAssign.window.text:SetText( strsub(text, 1, position)..color..strsub(text, position+1, string.len(text) ))
		AngryAssign.window.text.editBox:SetCursorPosition( position + string.len(color), string.len(text) )
	else
		AngryAssign.window.text:SetText( color..AngryAssign.window.text:GetText())
		AngryAssign.window.text.editBox:SetCursorPosition( string.len(color) )
	end
...
function AngryAssign:CreateColorPicker()
	local window = AceGUI:Create("Window")
	window:SetTitle("Insert a Color")
	window:SetLayout("List")
	window:SetWidth(235)
	window:SetHeight(310)
	window.frame:SetParent(self.window.frame)
	window.frame:ClearAllPoints()
	window.frame:SetPoint("TOPRIGHT", self.window.frame, "TOPLEFT", 0, 0, 4, 0)
	window.frame:SetMovable(false)
	window.title:SetScript("OnMouseDown", nil)
	window.title:SetScript("OnMouseUp", nil)
	window:EnableResize(false)
	...
	local colors_basic = {}
	local colors_class = {}
	-- Angry Assignment color: Button text
	-- Basic colors
	colors_basic[1] = {"|cred", "|cffeb310cRed"}
	colors_basic[2] = {"|cgreen", "|cff0adc00Green"}
	colors_basic[3] = {"|cblue", "|cff00cbf4Blue"}
	colors_basic[4] = {"|cyellow", "|cfffaf318Yellow"}
	colors_basic[5] = {"|corange", "|cffff9d00Orange"}
	colors_basic[6] = {"|cpink", "|cfff64c97Pink"}
	colors_basic[7] = {"|cpurple", "|cffdc44ebPurple"}
	-- Class colors
	colors_class[1] = {"|cdruid", "|cffeb310cDruid"}
	colors_class[2] = {"|chunter", "|cffabd473Hunter"}
	colors_class[3] = {"|cmage", "|cff40C7ebMage"}
	colors_class[4] = {"|cpaladin", "|cfff58cbaPaladin"}
	colors_class[5] = {"|cpriest", "|cffffffffPriest"}
	colors_class[6] = {"|crogue", "|cfffff569Rogue"}
	colors_class[7] = {"|cshaman", "|cff0070deShaman"}
	colors_class[8] = {"|cwarlock", "|cff8787edWarlock"}
	colors_class[9] = {"|cwarrior", "|cffc79c6eWarrior"}
	colors_class[10] = {"|cdeathknight", "|cffc41e3aDeath Knight"}

	local group_colors_basic = AceGUI:Create("SimpleGroup")
	group_colors_basic:SetLayout("Flow")
	group_colors_basic:SetFullWidth(true)

	local buttons_wdith = 100
	local buttons_height = 20

	for _, color in ipairs(colors_basic) do
		local clr_button = AceGUI:Create("Button")
		clr_button:SetText(color[2])
		clr_button:SetWidth(buttons_wdith)
		clr_button:SetHeight(buttons_height)
		clr_button:SetUserData('color', color[1])
		clr_button:SetCallback("OnClick", AngryAssign_ColorPicker_Clicked)
		group_colors_basic:AddChild(clr_button)
	end
	window:AddChild(group_colors_basic)

	local heading_groups = AceGUI:Create("Heading")
	heading_groups:SetFullWidth(true)
	window:AddChild(heading_groups)

	local group_colors_class = AceGUI:Create("SimpleGroup")
	group_colors_class:SetLayout("Flow")
	group_colors_class:SetFullWidth(true)

	for _, color in ipairs(colors_class) do
		local clr_button = AceGUI:Create("Button")
		clr_button:SetText(color[2])
		clr_button:SetWidth(buttons_wdith)
		clr_button:SetHeight(buttons_height)
		clr_button:SetUserData('color', color[1])
		clr_button:SetCallback("OnClick", AngryAssign_ColorPicker_Clicked)
		group_colors_class:AddChild(clr_button)
	end
	window:AddChild(group_colors_class)
	...
	reset_button:SetText("Reset color")
	reset_button:SetWidth(120)
	reset_button:SetHeight(buttons_height)
	reset_button:SetUserData('color', "||r")
	reset_button:SetCallback("OnClick", AngryAssign_ColorPicker_Clicked)

	window:AddChild(reset_button)
end
...
function AngryAssign:UpdateSelected(destructive)
	...
	if page and permission then
		...
		self.window.button_icons:SetDisabled(false)
		self.iconpicker:Show()
		self.window.button_colors:SetDisabled(false)
		...
		self.window.button_icons:SetDisabled(true)
		self.window.button_colors:SetDisabled(true)
...
function AngryAssign:UpdateDisplayed()
	...
	if page then
		...
		text = text:gsub("||", "|")
			...
			:gsub(ci_pattern('{hs}'), "|TInterface\\Icons\\INV_Stone_04:0:0:0:0:64:64:4:60:4:60|t")
			:gsub(ci_pattern('{bloodlust}'), "{bl}")
			:gsub(ci_pattern('{bl}'), "|TInterface\\Icons\\SPELL_Nature_Bloodlust:0:0:0:0:64:64:4:60:4:60|t")
			:gsub(ci_pattern('{icon%s+(%d+)}'), function(id)
				return format("|T%s:0:0:0:0:64:64:4:60:4:60|t", select(3, GetSpellInfo(tonumber(id))))
			end)
			:gsub(ci_pattern('{icon%s+([%w_]+)}'), "|TInterface\\Icons\\%1:0:0:0:0:64:64:4:60:4:60|t")
			...
			:gsub(ci_pattern('{heroism}'), "|TInterface\\Icons\\ABILITY_Shaman_Heroism:0:0:0:0:64:64:4:60:4:60|t")
