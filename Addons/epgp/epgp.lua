...
local function ZeroGPAwardPopup(log_callback, name, reason, amount, mass, undo)
    local dialog = StaticPopup_Show("EPGP_LOG_ZERO_GP", name)
    if dialog then
      dialog.log_callback = log_callback
      dialog.name = name
...
function EPGP:IncGPBy(name, reason, amount, mass, undo)
  ...
  -- For 0 GP awards we might not want to log them
  if amount == 0 and not undo then
    callbacks:Fire("ZeroGPAward", name, reason, amount, mass, undo)
    ZeroGPAwardPopup("ZeroGPAwardDelayedLog", name, reason, amount, mass, undo)
  elseif amount then
...
-- New: Banked item that also logs the player name
function EPGP:BankItemPlayer(name, reason, undo)
  callbacks:Fire("BankedItem", GUILD_BANK.." ("..name..")", reason, 0, false, undo)
  if not undo then
    ZeroGPAwardPopup("BankedItemDelayedLog", GUILD_BANK.." ("..name..")", reason, 0, false, undo)
  end
end

-- New: Disenchanted item that also logs the player name
function EPGP:DisenchantItemPlayer(name, reason, undo)
  callbacks:Fire("BankedItem", "Disenchant".." ("..name..")", reason, 0, false, undo)
  if not undo then
    ZeroGPAwardPopup("BankedItemDelayedLog", "Disenchant".." ("..name..")", reason, 0, false, undo)
  end
