...
function mod:ZeroGPAward(event_name, name, reason, amount, mass)
  if mass then return end
  mod:Announce(L["%+d GP (%s) to %s"], amount, reason, name)
end

-- Calling this will create a log entry for 0 GP.
function mod:ZeroGPAwardDelayedLog(event_name, name, reason, amount, mass)
end
...
function mod:BankedItemDelayedLog(event_name, name, reason, amount, mass)
end
...
mod.optionsArgs = {
  ...
  events = {
    ...
    values = {
      ...
      ZeroGPAward = L["A member is credited GP"],
      ZeroGPAwardDelayedLog = L["A member is credited GP"],
      BankedItem = L["An item was disenchanted or deposited into the guild bank"],
      BankedItemDelayedLog = L["An item was disenchanted or deposited into the guild bank"],
      Decay = L["EPGP decay"],
