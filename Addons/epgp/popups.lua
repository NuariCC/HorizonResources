...
StaticPopupDialogs["EPGP_LOG_ZERO_GP"] = {
  text = "Do you want to log the 0 GP award?",
  button1 = YES,
  button2 = NO,
  timeout = 0,
  hideOnEscape = 1,
  whileDead = 1,
  OnAccept = function(self)
               EPGP.callbacks:Fire(self.log_callback, self.name, self.reason, self.amount, self.mass, self.undo)
             end,
  
  OnUpdate = function(self, elapsed)
              if EPGP:CanIncGPBy(self.reason, 0) then
                self.button1:Enable()
              else
                self.button1:Disable()
              end
