...
function LootMasterML:HandleEPGPCommand(command, message, sender, event)
    ...
    if command=='PAS' or command=='PASS' or command=='NEED' or command=='GREED' or command=='UPGRADE' or command=='OFFSPEC' and message~='' then
        ...
        self:SendWhisperResponse( format('"%s" not understood. usage: /w %s -epgp need/upgrade/offspec/greed/pass [itemlink]', command, UnitName('player')), sender );
...
local commandPatterns = {
    ...
    '^%s*-([gG][rR][eE][eE][dD])%s*(.*)',
    '^%s*-([pP][aA][sS][sS]?)%s*(.*)',
    '^%s*-([uU][pP][gG][rR][aA][dD][eE]?)%s*(.*)',
    '^%s*-([oO][fF][fF][sS][pP][eE][cC]?)%s*(.*)'
}
...
function LootMasterML:AskCandidateIfNeeded( link, candidate )
	...
	if candidate == 'RAID' or candidate == 'PARTY' then
        ...
        SendChatMessage( format('%splease whisper me -epgp need/upgrade/offspec/greed/pass %s  (or use the popup if you have EPGPLootmaster installed)', MsgPrefix or '', loot.link or ''), candidate );
    ...
    elseif candidate ~= UnitName('player') then
        ...
        SendChatMessage( format('%splease whisper me -epgp need/upgrade/offspec/greed/pass %s  (or use the popup if you have EPGPLootmaster installed)', MsgPrefix or '', loot.link or ''), 'WHISPER', nil, candidate );
...
function LootMasterML:CHAT_MSG_LOOT( event, message )
    ...
    if lootType==LootMaster.LOOTTYPE.GP then -- and lootGP~=0 then
    ...
    -- New: Log banked items
    elseif lootType==LootMaster.LOOTTYPE.BANK then
        if CanEditOfficerNote() and EPGP and EPGP.BankItemPlayer then
            EPGP.BankItemPlayer(EPGP, tostring(sPlayer) or 'nil', tostring(sLink) or 'nil')
        else
            self:Print( format("<ERROR> Could not increase GP in officernotes for %s %s (EPGP not installed or no rights?!)", sPlayer or 'unknown player', sLink or 'unknown loot') )
        end
    -- New: Log disenchanted items
    elseif lootType==LootMaster.LOOTTYPE.DISENCHANT then
        if CanEditOfficerNote() and EPGP and EPGP.DisenchantItemPlayer then
            EPGP.DisenchantItemPlayer(EPGP, tostring(sPlayer) or 'nil', tostring(sLink) or 'nil')
        else
            self:Print( format("<ERROR> Could not increase GP in officernotes for %s %s (EPGP not installed or no rights?!)", sPlayer or 'unknown player', sLink or 'unknown loot') )
        end
    end;
